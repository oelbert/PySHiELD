...
def monin_obukhov_similarity(
    ...
    if (abs(1.0 - tem1) > 1.0e-6):
        ztmax1 = - physcons.BETA * log(tem1) / (physcons.ALPHA2 * (1. - tem1))
    else:
        ztmax1 = 99.0
    ...
    dtv = sign(1., dtv) * adtv
    rb = max(-5000.0, (constants.GRAV + constants.GRAV) * dtv * z1 / (
        (thv1 + tvs) * wind * wind)
    )
    tem1 = 1.0 / z0max
    tem2 = 1.0 / ztmax
    ...
    if dtv >= 0.0:
        ...
        if hlinf > .25:
            ...
            hl0inf = z0max * tem1
            hltinf = ztmax * tem1
            aa = sqrt(1. + physcons.ALPHA4 * hlinf)
            aa0 = sqrt(1. + physcons.ALPHA4 * hl0inf)
            bb = aa
            bb0 = sqrt(1. + physcons.ALPHA4 * hltinf)
        ...
        if hlinf >= -0.5:
            hl1 = hlinf
            pm = (physcons.A0 + physcons.A1 * hl1) * hl1 / (
                1. + (physcons.B1 + physcons.B2 * hl1) * hl1
            )
            ph = (physcons.A0P + physcons.A1P * hl1) * hl1 / (
                1. + (physcons.B1P + physcons.B2P * hl1) * hl1
            )
            hl110 = hl1 * 10. * z1i
            hl110 = min(max(hl110, physcons.ZTMIN), ztmax1)
            pm10 = (physcons.A0 + physcons.A1 * hl110) * hl110 / (
                1. + (physcons.B1 + physcons.B2 * hl110) * hl110
            )
            hl12 = (hl1 + hl1) * z1i
            hl12 = min(max(hl12, physcons.ZTMIN), ztmax1)
            ph2 = (physcons.A0P + physcons.A1P * hl12) * hl12 / (
                1. + (physcons.B1P + physcons.B2P * hl12) * hl12
            )
        else:  # hlinf < 0.05
            hl1 = -hlinf
            tem1 = 1.0 / sqrt(hl1)
...
def cal_z0_hwrf15(ws10m):
    ...
    elif (ws10m > 5.0) and (ws10m <= 10.):
        z0 = 0.00000235*(ws10m**2-25.)+3.805129199617346e-05
    elif (ws10m > 10.0) and (ws10m <= 60.):
        z0 = a6 + a5*ws10m + a4*ws10m**2 + a3*ws10m**3 + a2*ws10m**4 + a1*ws10m**5 + a0*ws10m**6
    else:
        z0 = b6 + b5*ws10m + b4*ws10m**2 + b3*ws10m**3 + b2*ws10m**4 + b1*ws10m**5 + b0*ws10m**6
    return z0
...
def cal_zt_hwrf15(ws10m, zt):
    ...
    elif (ws10m > 7.0) and (ws10m <= 15.):
        zt = a6 + a5*ws10m + a4*ws10m**2 + a3*ws10m**3 + a2*ws10m**4 + a1*ws10m**5 + a0*ws10m**6
    elif (ws10m > 15.0) and (ws10m <= 60.):
        zt = b6 + b5*ws10m + b4*ws10m**2 + b3*ws10m**3 + b2*ws10m**4 + b1*ws10m**5 + b0*ws10m**6
    else:
        zt = c6 + c5*ws10m + c4*ws10m**2 + c3*ws10m**3 + c2*ws10m**4 + c1*ws10m**5 + c0*ws10m**6
    return zt
...
def cal_z0_hwrf17(ws10m):
    ...
    if ws10m <= 6.5:
        ...
        z0 = p25*ws10m**5 + p24*ws10m**4 + p23*ws10m**3 + p22*ws10m**2 + p21*ws10m + p20
    elif (ws10m > 15.7) and (ws10m <= 53.):
        z0 = exp( p35*ws10m**5 + p34*ws10m**4 + p33*ws10m**3 + p32*ws10m**2 + p31*ws10m + p30 )
    else:
         z0 = p40
    return z0
...
def cal_zt_hwrf17(ws10m):
    ...
    if (ws10m >= 0.0) and (ws10m < 5.9):
        ...
        zt = p10 + ws10m * (p11 + ws10m * (p12 + ws10m * (p13 + ws10m * (p14 + ws10m * p15))))
    elif (ws10m > 15.4) and (ws10m <= 21.6):
        zt = p20 + ws10m * (p21 + ws10m * (p22 + ws10m * (p23 + ws10m * (p24 + ws10m * p25))))
    elif (ws10m > 21.6) and (ws10m <= 42.2):
        zt = p30 + ws10m * (p31 + ws10m * (p32 + ws10m * (p33 + ws10m * (p34 + ws10m * p35))))
    elif ( ws10m > 42.2) and (ws10m <= 53.3):
        zt = p40 + ws10m * (p41 + ws10m * (p42 + ws10m * (p43 + ws10m * (p44 + ws10m * p45))))
    elif ( ws10m > 53.3) and (ws10m <= 80.0):
        zt = p50 + ws10m * (p51 + ws10m * (p52 + ws10m * (p53 + ws10m * (p54 + ws10m * (p55 + ws10m * p56)))))
    elif ( ws10m > 80.0):
        zt = p60
    return zt
...
def cal_z0_moon(ws10m):
    ...
    z0_adj = 0.001*(0.085*wind_th_moon - 0.58) - (charnock/constants.GRAV)*ustar_th*ustar_th 
...
def sfc_diff(
    ...
    """
    from __externals__ import do_z0_hwrf15, do_z0_hwrf17, do_z0_hwrf17_hwonly, do_z0_moon
    ...
    with computation(PARALLEL), interval(...):
        ...
        if flag_iter[0, 0]:
            ...
            tem1 = 1.0 + constants.ZVIR * max(q1, 1.e-8)
            thv1 = t1 * prslki * tem1
...
class SurfaceExchange:
    def __init__(
        ...
        """
        assert sum([config.do_z0_hwrf15, config.do_z0_hwrf17, config.do_z0_hwrf17_hwonly, config.do_z0_moon]) == 1, "sfc_diff: exactly one ocean surface option must be enabled"
